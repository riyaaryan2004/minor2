...
print("Train R2 (Mood):", mood_model.score(X_train, y_train_mood))

print("\nProductivity CV Mean:", prod_scores.mean())
print("Train R2 (Productivity):", prod_model.score(X_train, y_train_prod))
...
for name, score in sorted(
    zip(feature_names, prod_model.feature_importances_),
    key=lambda x: x[1],
    reverse=True
):
    print(f"{name}: {score:.4f}")
...
if os.path.exists(txt_file):
    ...
    if "Model: LightGBM" in content:
        parts = content.split("Model: LightGBM")
        new_content = parts[0]
    else:
        new_content = content
